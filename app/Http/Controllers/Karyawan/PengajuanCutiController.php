...
class PengajuanCutiController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Mengambil seluruh data jenis cuti untuk ditampilkan pada filter dropdown
        $jenisCutiList = JenisCuti::all();

        // Membuat query pengajuan cuti milik user yang sedang login beserta relasi user dan jenis cuti
        $query = PengajuanCuti::where('user_id', $user->id)
            ->with(['user', 'jenisCuti'])
            ->latest();

        // Jika user memilih filter jenis cuti, maka query akan menampilkan data sesuai jenis cuti yang dipilih
        if ($request->filled('jenis_cuti_id')) {
            $query->where('jenis_cuti_id', $request->jenis_cuti_id);
        }

        // Menjalankan query dan membatasi tampilan sebanyak 10 data per halaman.
        $pengajuanCuti = $query->paginate(10)->withQueryString();

        // Mengirim data pengajuan cuti dan daftar jenis cuti ke halaman index pengajuan cuti
        return view('karyawan.pengajuan_cuti.index', compact('pengajuanCuti', 'jenisCutiList'));
    }
    ...
    public function create()
    {
        ...
        return view('karyawan.pengajuan_cuti.create', compact('jenisCuti', 'hakCuti'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()
            ->route('karyawan.pengajuan_cuti.index')
    ...
    /**
     * Calculate tanggal masuk (first working day after tanggal_selesai)
